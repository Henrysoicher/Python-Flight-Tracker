...
# Inning status is drawn at top right (same font height as teams)
# Padres line colors only the letters S (brown) and D (yellow). Everything else is white.
...
# New: a slightly larger font list (kept for compatibility; will fall back safely)
...
# Baselines used by MLB view (work fine with 6x10 as well)
MLB_LINE1_Y = 13
MLB_LINE2_Y = 28
...
# ===== Padres cache =====
# Includes per-line colors and inning text (colors now ignored to keep code minimal)
_padres_cache = {
    "ts": 0.0, "have": False,
    "top": "", "bottom": "", "corner": "",
...
# Padres brand colors for letter accents
PADRES_BROWN = graphics.Color(47, 36, 29)   # tweak if LEDs need
...
def fetch_padres_score_lines():
    ...
    Output is have_game, top_line, bottom_line, corner_text, top_color, bottom_color (colors ignored).
...
# ===== MLB renderer using SAME font for teams & inning; Padres S/D coloring only =====
def render_mlb_view(matrix: RGBMatrix,
                    font_small, font_big,  # keep signature for minimal diff; we'll use font_small for both
                    top_text: str, bottom_text: str, corner_text: str,
                    top_color: graphics.Color, bottom_color: graphics.Color,  # ignored; kept for compatibility
                    secs: float, margin: int):
    end_time = time.time() + secs
    c = matrix.CreateFrameCanvas()

    # use the same font for everything to avoid font availability issues
    font = font_small

    def width(t: str) -> int:
        return graphics.DrawText(c, font, 0, 0, graphics.Color(0,0,0), t or "")

    w_corner = width(corner_text or "")
    ...
    while time.time() < end_time:
        ...
        # Padres top line with S/D colored, other chars white
        draw_colored_padres(margin, MLB_LINE1_Y, top_text or "")

        # Inning at top right in SAME font, white
        if corner_text:
            xc = matrix.width - margin - w_corner
            graphics.DrawText(c, font, xc, MLB_LINE1_Y, WHITE, corner_text)

        # Opponent bottom line in white
        graphics.DrawText(c, font, margin, MLB_LINE2_Y, WHITE, bottom_text or "")

        matrix.SwapOnVSync(c)
        time.sleep(0.05)  # light refresh
...
def load_medium_font():
    # kept for compatibility; not required since render uses small font
...
def main():
    ...
    font_mlb = load_medium_font()  # will fall back to small; not used
    matrix = setup_matrix()
    ...
    while True:
        try:
            ...
            if have_game:
                # MLB view: same font height for teams & inning; Padres S/D colored, opponent white
                render_mlb_view(matrix, font_small, font_small,
                                top_line or "PADRES", bottom_line or "",
                                corner_text or "",
                                top_color or WHITE, bottom_color or WHITE,  # ignored
                                POLL_INTERVAL_SEC, SIDE_MARGIN_PX)
            else:
